...
import (
	...
	"github.com/viletay/provider-vkcs/apis"
	"github.com/viletay/provider-vkcs/apis/v1alpha1"
	"github.com/viletay/provider-vkcs/config"
...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for vkcs").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-vkcs"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-vkcs",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup vkcs controllers")
